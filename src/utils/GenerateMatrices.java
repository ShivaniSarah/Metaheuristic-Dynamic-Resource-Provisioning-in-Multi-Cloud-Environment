package cloudsample.utils;


//import org.cloudbus.cloudsim.examples.utils.*;
import java.io.*;

public class GenerateMatrices {
    private static double[][] commMatrix, execMatrix;
    private File commFile = new File("LOACommunicationTimeMatrix.txt");
    private File execFile = new File("LOAExecutionTimeMatrix.txt");
    ...
    private void initCostMatrix() throws IOException {
        ...
        for (int i = 0; i < Constants.NO_OF_TASKS; i++) {
            for (int j = 0; j < Constants.NO_OF_DATA_CENTERS; j++) {
                commMatrix[i][j] = Math.random() * 300 + 20;
                execMatrix[i][j] = Math.random() * 250 + 10;
